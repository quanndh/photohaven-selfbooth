...
class FolderWatcher:
    ...
    def _move_file_with_retry(self, source: Path, destination: Path, max_retries: int = 5, retry_delay: float = 1.0) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                # Wait for file to be accessible before moving (only on first few attempts)
                if attempt < 2:
                ...
                if "being used by another process" in error_msg or "used by another process" in error_msg or "the process cannot access" in error_msg:
                    if attempt < max_retries - 1:
    ...
    def _process_image(self, folder_path, folder_name: str, image_path: str):
        ...
        try:
            ...
            if not self._move_file_with_retry(image_file, original_destination):
                logger.error(f"Failed to move original image {image_file.name} after retries - file may still be locked")
                return
